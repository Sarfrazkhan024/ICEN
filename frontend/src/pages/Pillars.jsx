      ...
      <div className="max-w-[1300px] mx-auto px-6 lg:px-10">
        ...
        <div className="mt-16 border border-icen-line divide-y divide-icen-line bg-icen-paper">
          {PILLARS.map((p) => {
            ...
            return (
              <div key={p.id} className="group grid grid-cols-[80px_1fr_auto] md:grid-cols-[160px_1fr_auto] gap-6 items-center p-6 md:p-8 hover:bg-icen-ivory transition-colors" data-testid={`pillar-row-${p.id}`}>
                <div className="font-mono text-xs md:text-sm text-icen-muted tracking-widest">PILLAR {String(p.id).padStart(2, '0')}</div>
                <div>
                  <h3 className="font-serif text-2xl md:text-3xl text-icen-ink leading-tight">{p.title}</h3>
                  <p className="mt-2 text-[15px] text-icen-inkSoft leading-relaxed max-w-3xl">{p.desc}</p>
                </div>
